...
export default function LawSideBar({ className }: SideBarProps) {
  const [isCollapsed, setIsCollapsed] = useState(false);
  const [isTransitioning, setIsTransitioning] = useState(false);
  const [expandedSection, setExpandedSection] = useState<'ai-counsel' | 'cases' | null>(null);
  const [mobileExpandedSection, setMobileExpandedSection] = useState<'ai-counsel' | 'cases' | null>(null);
  ...
  const { isLoading: isLoadingHistory, lensChatHistories, fetchAllLensChatHistory, deleteLensChatHistory } =
    useLensChat();
  const { cases, isLoading: isLoadingCases, fetchCases, setFirmId } = useCases();
  ...
  // Fetch firm and cases data
  useEffect(() => {
    const fetchFirmAndCases = async () => {
      try {
        const response = await fetch('/api/firms/my');
        if (response.ok) {
  ...
  }, [session?.user?.id, fetchCases, setFirmId]);
  ...
  const menuItems = [
    {
      icon: Building,
      label: "事務所",
      href: "/self-firm/management",
    },
    {
      icon: ScrollText,
      label: "合約管理",
      href: "/self-firm/contracts",
    },
    {
      icon: FileText,
      label: "案例列表",
      href: "/self-firm/cases",
    },
    {
      icon: Bot,
      label: "AI Counsel",
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "relative z-200 hidden lg:flex h-screen flex-col border-r border-sidebar-border bg-secondary transition-all duration-300",
          isCollapsed
                ...
                if (item.label === "AI Counsel") {
                  return (
                    <div key={index} className="space-y-1">
                      <div
                        className="flex w-full items-center justify-between space-x-3 p-3 rounded-lg hover:bg-muted/20 transition-colors duration-200 cursor-pointer"
                        onClick={() => setExpandedSection(expandedSection === 'ai-counsel' ? null : 'ai-counsel')}
                      >
                        ...
                        <ChevronDown className={`h-4 w-4 text-secondary-foreground/60 transition-transform duration-300 ${expandedSection === 'ai-counsel' ? 'rotate-0' : '-rotate-90'}`} />
                      </div>
                      <div
                        className="flex ml-8 cursor-pointer items-center justify-between rounded-md border border-primary/50 bg-linear-to-r from-black/80 to-primary/10 p-3 transition-all duration-300 hover:border-primary hover:from-primary/5 hover:to-primary/20 hover:shadow-[0_0_10px_rgba(90,90,255,0.3)]"
                        onClick={() => router.push(`/self-firm/legal-agent`)}
                      >
                      ...
                      <div className={`ml-8 overflow-hidden transition-all duration-500 ease-in-out ${expandedSection === 'ai-counsel' ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'
                        }`}>
                        <div className="custom-scrollbar max-h-[calc(50vh-200px)]  space-y-3 overflow-y-auto overflow-x-hidden">
                  ...
                  return (
                    <div key={index} className="space-y-1">
                      ...
                      >
                        ...
                        <ChevronDown className={`h-4 w-4 text-secondary-foreground/60 transition-transform duration-300 ${expandedSection === 'cases' ? 'rotate-0' : '-rotate-90'
                          }`} />
                      </div>
                      <div
                        className="flex ml-8 cursor-pointer items-center justify-between rounded-md border border-primary/50 bg-linear-to-r from-black/80 to-primary/10 p-3 transition-all duration-300 hover:border-primary hover:from-primary/5 hover:to-primary/20 hover:shadow-[0_0_10px_rgba(90,90,255,0.3)]"
                        onClick={() => router.push(`/self-firm/cases/new`)}
                      >
                        <div className="flex items-center">
                          <span className="text-nowrap font-basis-mono text-sm font-medium tracking-wide text-primary">
                            新增案例
                      ...
                      <div className={`ml-8 overflow-hidden transition-all duration-500 ease-in-out ${expandedSection === 'cases' ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'
                        }`}>
                        <div className="custom-scrollbar max-h-[calc(50vh-220px)] space-y-3 overflow-y-auto overflow-x-hidden">
                          {isLoadingCases ? (
                            <div className="text-sm text-secondary-foreground/60 p-2 flex items-center space-x-2">
                              <div className="animate-spin rounded-full h-3 w-3 border-b border-primary"></div>
                              <span>載入案例中...</span>
                            ...
                            cases.map((caseItem) => (
                              <div
                                key={caseItem.id}
                                onClick={() => router.push(`/self-firm/cases/${caseItem.id}`)}
                                className="flex items-center cursor-pointer justify-between p-2 rounded-md hover:bg-muted/20 transition-colors group"
                                title={caseItem.title}
                              >
                                <div className="flex flex-col">
                                  <span className="max-w-[180px] truncate text-nowrap text-xs lg:text-sm text-white/80">
                                    {caseItem.title}
                                  </span>
                                  <div className="flex items-center gap-2 mt-1">
                                    ...
                                    <span className={`text-xs px-1.5 py-0.5 rounded text-white/70 ${caseItem.status === 'active' ? 'bg-green-600/20' :
                                      caseItem.status === 'closed' ? 'bg-gray-600/20' :
                                        caseItem.status === 'paused' ? 'bg-yellow-600/20' :
                                          'bg-blue-600/20'
                                      }`}>
                                      {caseItem.status === 'active' ? '進行中' :
                                        caseItem.status === 'closed' ? '已結案' :
                                          caseItem.status === 'paused' ? '暫停' :
                                            '待審理'}
                                    </span>
                                  </div>
                                </div>
                                {caseItem.is_priority && (
              ...
              if (item.label === "AI Counsel") {
                return (
                  <div key={index} className="space-y-1">
                    <div
                      className="flex w-full items-center justify-between space-x-3 p-3 rounded-lg hover:bg-muted/20 transition-colors duration-200 cursor-pointer"
                      onClick={() => setMobileExpandedSection(mobileExpandedSection === 'ai-counsel' ? null : 'ai-counsel')}
                    >
                      <div className="flex items-center space-x-3">
                        <item.icon className="h-5 w-5 text-secondary-foreground/60" />
                        <span className="text-nowrap text-secondary-foreground font-medium">
                      ...
                      <ChevronDown className={`h-4 w-4 text-secondary-foreground/60 transition-transform duration-300 ${mobileExpandedSection === 'ai-counsel' ? 'rotate-0' : '-rotate-90'}`} />
                    </div>
                    <div
                      className="flex ml-8 cursor-pointer items-center justify-between rounded-md border border-primary/50 bg-linear-to-r from-black/80 to-primary/10 p-3 transition-all duration-300 hover:border-primary hover:from-primary/5 hover:to-primary/20 hover:shadow-[0_0_10px_rgba(90,90,255,0.3)]"
                      onClick={() => {
                    ...
                    <div className={`ml-8 overflow-hidden transition-all duration-500 ease-in-out ${mobileExpandedSection === 'ai-counsel' ? 'max-h-64 opacity-100' : 'max-h-0 opacity-0'
                      }`}>
                      <div className="custom-scrollbar max-h-64 space-y-3 overflow-y-auto overflow-x-hidden">
                        {isLoadingHistory ? (
                          <div className="text-sm text-secondary-foreground/60 p-2 flex items-center space-x-2">
                            <div className="animate-spin rounded-full h-3 w-3 border-b border-primary"></div>
                            <span>載入聊天歷史中...</span>
                          ...
                          lensChatHistories.filter(chat => !chat.user_case_id).slice(0, 5).map((chat) => (
                            <div onClick={() => {
                              router.push(`/self-firm/legal-agent/p/${chat.sessionId}`);
                              setIsMobileSidebarOpen(false);
                            }}
              ...
              if (item.label === "案例列表") {
                return (
                  <div key={index} className="space-y-1">
                    <div
                      className="flex w-full items-center justify-between space-x-3 p-3 rounded-lg hover:bg-muted/20 transition-colors duration-200 cursor-pointer"
                      onClick={() => setMobileExpandedSection(mobileExpandedSection === 'cases' ? null : 'cases')}
                    >
                      <div className="flex items-center space-x-3">
                        <item.icon className="h-5 w-5 text-secondary-foreground/60" />
                        <span className="text-nowrap text-secondary-foreground font-medium">
                      ...
                      <ChevronDown className={`h-4 w-4 text-secondary-foreground/60 transition-transform duration-300 ${mobileExpandedSection === 'cases' ? 'rotate-0' : '-rotate-90'}`} />
                    </div>
                    <div
                      className="flex ml-8 cursor-pointer items-center justify-between rounded-md border border-primary/50 bg-linear-to-r from-black/80 to-primary/10 p-3 transition-all duration-300 hover:border-primary hover:from-primary/5 hover:to-primary/20 hover:shadow-[0_0_10px_rgba(90,90,255,0.3)]"
                      onClick={() => {
                        router.push(`/self-firm/cases/new`);
                        setIsMobileSidebarOpen(false);
                      }}
                    >
                      <div className="flex items-center">
                    ...
                    <div className={`ml-8 overflow-hidden transition-all duration-500 ease-in-out ${mobileExpandedSection === 'cases' ? 'max-h-64 opacity-100' : 'max-h-0 opacity-0'
                      }`}>
                      <div className="custom-scrollbar max-h-64 space-y-3 overflow-y-auto overflow-x-hidden">
                        {isLoadingCases ? (
                          <div className="text-sm text-secondary-foreground/60 p-2 flex items-center space-x-2">
                          ...
                          cases.slice(0, 5).map((caseItem) => (
                            <div
                              key={caseItem.id}
                              onClick={() => {
                                router.push(`/self-firm/cases/${caseItem.case_number}`);
                                setIsMobileSidebarOpen(false);
                              }}
                              className="flex items-center cursor-pointer justify-between p-2 rounded-md hover:bg-muted/20 transition-colors group"
                              title={caseItem.title}
